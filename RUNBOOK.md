...
| 2026-07-18 | **New "Inside the Shop" section + 4 in-store merchandising photos live on prd (squash commit b978872 → prd).** Josh dropped 4 iPhone photos of in-store merchandising: glass-door Wisconsin cheese cooler (Roth/Yodelay/Widmer/North Country/Alpine Lace), Crossroads-branded Original Beef Strips + Teriyaki Beef Strips on the shelf above a cheese selection, dry-goods/condiments shelf (industrial pipe shelving against the fieldstone wall — BBQ sauces, rubs, hot sauces, honey, bread buns), and the backlit Victorinox retail knife display. Plan: optimize all 4 (PIL LANCZOS resize, JPEG q85, progressive, sharpen), drop the price-sticker row from the knife photo (bottom 36% cropped — Swiss Classic boxed sets remain visible without the $42.95/$36.80 stickers), retitled stone-wall shelf to `dry-goods-condiments-shelf.jpg` (it's the dry-goods shelf, not retail knives), wire into 3 places on index.html: (1) new 4-tile section between "What We Carry" and "Three Rooms" with caption + title + 2-sentence description per tile, (2) marquee gallery extended 12→16 photos in BOTH strips so the seamless carousel loop stays in sync, (3) swap `stock-cheese.jpg`→`cheese-cooler.jpg` on the Wisconsin Cheese card and `stock-snack-sticks.jpeg`→`jerky-cheese-shelf.jpg` on the Snack Sticks card. New section CSS: `.shop-grid-tiles` grid `repeat(4,1fr)` desktop → 2-col tablet @1100px → 1-col mobile @640px, portrait frame modifier for the dry-goods tile (`aspect-ratio: 3/4`), mirrors the existing `.rooms` styling for visual consistency. Background uses `--bg-alt` (warm cream) to differentiate from the white "What We Carry" above. Dev push `b978872`, squash-merged to prd. Verified: all 4 assets 200 on apex, knife sticker crop verified — boxed sets still on the rack. |
