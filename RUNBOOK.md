...
> **Status:** Maintenance
> **Next:** Review the recovered `Inside the Shop` section on dev, then promote dev → prd only with Josh's explicit approval. The four image files survived, but merge commit `b978872` dropped the section, product-card swaps, and marquee references from `9248e51`; recovered on 2026-07-20 with retail-only wording preserved.
> **Blocker:** None
