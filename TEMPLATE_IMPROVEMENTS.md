...
## [2026-07-03] Issue: Animated grid + grain noise overlays too busy for non-tech clients

- **Found during:** Denelie's Pizza (Spooner WI restaurant) build
- **Problem:** Template shipped two decorative overlays: (1) `body::before` SVG fractal-noise grain texture covering the entire page at 3.5% opacity, and (2) `.hero-grid` animated diagonal-grid pattern with 30s infinite scroll. Both read as "tech demo" visual language — fine for SaaS/dev-tool sites, but reads as cheap/dated on restaurant, retail, and lifestyle sites. Josh called them out explicitly: "remove that floating grid background. Also remove that from the template. Not needed on every site."
- **Fix applied:** Removed from `/workspace/premium-template/index.html`:
  - `body::before` grain noise CSS block (8 lines)
  - `.hero-grid` CSS + `@keyframes gridMove` (12 lines)
  - `<div class="hero-grid" aria-hidden="true">` element from hero markup
  - Same three removals applied to the in-flight Denelie's project copy
- **Decision rule for future client builds:** these decorations are a project-level choice, not template-level. If a future client (e.g. a SaaS or dev-tool company) explicitly wants the grid/grain aesthetic, add it back as a project-level CSS override, not a template default. Default off.
